...
class HTFHybridBot:
    ...
    def _check_liquidation_safety(self, leverage: int = 1) -> bool:
        ...
        if self.position == 1:  # LONG
            if liq_price >= self.sl_price - delta:
                logger.critical(
                    "⚠️ [Hybrid/Simulated] LIQUIDATION RISK: %s LONG liq=$%.2f >= SL-delta=$%.2f",
            ...
            if liq_price <= self.sl_price + delta:
                logger.critical(
                    "⚠️ [Hybrid/Simulated] LIQUIDATION RISK: %s SHORT liq=$%.2f <= SL+delta=$%.2f",
                    self.symbol, liq_price, self.sl_price + delta,
                )
                self._write_liquidation_alert(liq_price, delta)
                return False
    ...
    def run_iteration(self) -> Dict:
        ...
        try:
            ...
            # 2. Check hybrid exit logic before computing new action (uses cached BOS/CHOCH)
        ...
        def _on_message(ws, message):
            try:
                ...
                with self._lock:
                    if self.position == 0:
                        return
                    exit_reason = self._check_hybrid_exit(price)
                    if exit_reason:
                        logger.info("WS: exit triggered (%s) @ $%.2f", exit_reason, price)
                        if exit_reason == "PARTIAL_CLOSE":
                            self._execute_partial_close(price)
                        else:
