...
def main():
    ...
    with col_sidebar:
        ...
        # Fetch real-time price using 1m data for accuracy
        try:
            live_data = load_real_market_data(st.session_state.selected_asset, '1m')
            current_price = float(live_data.iloc[-1]['close']) if not live_data.empty else 0
        except:
            live_1h = load_real_market_data(st.session_state.selected_asset, '1h')
            current_price = float(live_1h.iloc[-1]['close']) if not live_1h.empty else 0
