#!/usr/bin/env python3
"""
HTF Partial Take-Profit Paper Trade Bot (Strategy 2)
=====================================================
Identical to live_trading_htf.py for ENTRY logic.
ONLY the exit logic differs: staged partial take-profit exits.

Exit rules:
  - At +1.5% unrealized → close 33% of position  (PARTIAL_CLOSE_1)
  - At +2.5% unrealized → close another 33%       (PARTIAL_CLOSE_2)
  - At +3.0% unrealized → close remainder          (CLOSE via TP)
  - SL at -1.5% on remaining position throughout

This bot runs ONLY in dry-run mode (paper trade).

Usage:
    python live_trading_htf_partial.py --dry-run --interval 900
"""
...
logger = logging.getLogger("htf_partial")

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
SYMBOL = "BTCUSDT"
STATE_FILE = Path("logs/htf_partial_state.json")
TRADES_FILE = Path("logs/htf_partial_trades.json")

# Validated walk-forward configuration (50% position, Sharpe 3.85)
POSITION_SIZE = 0.50   # 50% of balance per trade
STOP_LOSS_PCT = 0.015  # 1.5% stop loss
TAKE_PROFIT_PCT = 0.030  # 3.0% take profit (final exit)
TRADING_FEE = 0.0004   # 0.04% taker fee

# Partial take-profit thresholds
PARTIAL_TP1_PCT = 0.015  # +1.5% → close 33%
PARTIAL_TP2_PCT = 0.025  # +2.5% → close another 33%
PARTIAL_TP3_PCT = 0.030  # +3.0% → close remaining
...
# ---------------------------------------------------------------------------
# HTF Partial Take-Profit Bot
# ---------------------------------------------------------------------------

class HTFPartialBot:
    """
    Paper trading bot using Hierarchical Multi-Timeframe PPO agent
    with staged partial take-profit exits.
    ...
    def __init__(
        ...
        # No testnet executor for paper trade bot
        ...
        logger.info(
            "HTFPartialBot ready | symbol=%s dry_run=%s balance=%.2f",
    ...
    def _check_partial_tp(self, current_price: float) -> Optional[str]:
        """
        Partial take-profit exit logic.

        Stages:
          - +1.5% profit, partial_exits==0 → close 33% (PARTIAL_CLOSE_1)
          - +2.5% profit, partial_exits==1 → close 33% (PARTIAL_CLOSE_2)
          - +3.0% profit → close remaining  (TP)
          - -1.5% from entry → SL on remaining
        ...
        if profit_pct >= PARTIAL_TP3_PCT:
            return "TP"

        # --- Partial close 2: +2.5% and only 1 partial done ---
        if profit_pct >= PARTIAL_TP2_PCT and self.partial_exits == 1:
            return "PARTIAL_CLOSE_2"
    ...
    def _execute_partial_close(self, current_price: float, reason: str) -> Dict:
        """
        Execute a partial position close (33% of original units).

        Returns a trade dict for the partial close.
        """
        # Determine how many units to close (33% of original)
        close_units = self.original_units * 0.33
        ...
        # Match original _close_position balance logic, scaled to close_units:
        # balance += (price * units) + net_pnl - raw_pnl
        # Which simplifies to: balance += price * units - fee
        ...
        # Update partial exits counter
        if reason == "PARTIAL_CLOSE_1":
            self.partial_exits = 1
        elif reason == "PARTIAL_CLOSE_2":
        ...
        trade = {
            ...
            "original_units": self.original_units,
            "partial_exit_num": self.partial_exits,
            "pnl": net_pnl,
            "confidence": 1.0,
            "timestamp": datetime.now().isoformat(),
            "agent": "htf",
        ...
        logger.info(
            "📊 %s (%s) @ $%.2f | closed=%.5f remaining=%.5f | pnl=$%.2f | balance=$%.2f",
            action_str, reason, current_price, close_units, self.position_units,
            net_pnl, self.balance,
        )
    ...
    def _open_position(self, price: float, direction: int, confidence: float) -> Dict:
        ...
        # Reset partial take-profit state for new position
        self.partial_exits = 0
        ...
        logger.info(
            "📈 %s @ $%.2f | units=%.5f | SL=$%.2f | TP=$%.2f | conf=%.2f [strategy=partial]",
    ...
    def _close_position(self, price: float, reason: str, confidence: float) -> Dict:
        ...
        logger.info(
            "📉 %s @ $%.2f | pnl=$%.2f | reason=%s | balance=$%.2f [strategy=partial]",
        ...
        try:
            ...
            if exit_reason and self.position != 0:
                ...
                if exit_reason in ("PARTIAL_CLOSE_1", "PARTIAL_CLOSE_2"):
                    ...
                    trade = self._execute_partial_close(current_price, exit_reason)
            ...
            logger.info(
                "Step: price=$%.2f action=%s conf=%.2f pos=%d bal=$%.2f partial=%d [strategy=partial]",
                current_price, ACTION_LABELS[action], confidence, self.position,
                self.balance, self.partial_exits,
            )
...
def main():
    parser = argparse.ArgumentParser(description="HTF Partial Take-Profit Paper Trade Bot")
